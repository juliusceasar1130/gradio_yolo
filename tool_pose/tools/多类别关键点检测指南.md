...
### 核心文件
- `labelme_to_yolo_pose_multi_class.py` - 多类别转换工具（命令行版本）
- `labelme_to_yolo_pose_multi_class_config.py` - 多类别转换工具（配置文件版本，推荐）
- `labelme_to_yolo_pose_multi_class_config.yaml` - 配置文件模板
- `docs/pose姿态/多类别关键点检测指南.md` - 本指南文档
...
# 将配置文件放在脚本目录下，直接运行
cd datasets/tools
python labelme_to_yolo_pose_multi_class_config.py
```
...
```bash
# 使用自定义配置文件
python labelme_to_yolo_pose_multi_class_config.py /path/to/config.yaml
```

**配置文件示例** (`labelme_to_yolo_pose_multi_class_config.yaml`):

```yaml
# 路径配置
input_dir: ./datasets/datasets/tooltest_json
output_dir: ./datasets/datasets/tooltest_yolo
